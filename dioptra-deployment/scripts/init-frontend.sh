...
patch_vite_config() {
  local vite_config_file="${BUILD_DIR}/vite.config.ts"

  log_info "Patching ${vite_config_file} to adjust sassVariables"

  # backup original
  cp "${vite_config_file}" "${vite_config_file}.bak"

  # append import path from 'path' if not already present
  if ! grep -q "import path from 'path'" "${vite_config_file}"; then
    sed -i "/^import { quasar, transformAssetUrls } from '@quasar\/vite-plugin'/a import path from 'path'" "${vite_config_file}"
  fi

  # replace relative path with absolute path
  sed -i "s|sassVariables: 'src/quasar-variables.sass'|sassVariables: path.resolve(__dirname, 'src/quasar-variables.sass')|g" "${vite_config_file}"
}
